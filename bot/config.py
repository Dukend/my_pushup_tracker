...
class Settings:
    ...
    allowed_user_id: int = field(default_factory=lambda: int(os.getenv("ALLOWED_USER_ID", "0")))
    data_file: Path = field(
        default_factory=lambda: Path(os.getenv("DATA_FILE", "data/pushups.json"))
    )
    timezone: ZoneInfo = field(default_factory=lambda: ZoneInfo(os.getenv("TZ", "Europe/Moscow")))
